...
def runNoCalibInference(args, df_inf_data, df_val_inf_data, df_inf_data_device, threshold, n_branches_edge, savePath, overhead, calib_mode):
	...
	if(n_branches_edge == 1):
		...
		no_calib_inf_time2, a = spsa.compute_inference_time_multi_branches2(temp_list, n_branches_edge, max_exits, threshold, df_inf_data, df_inf_data_device, overhead)



	print(no_calib_inf_time, no_calib_inf_time2)
	sys.exit()
...
def main(args):
	...
	resultsPath = os.path.join(config.DIR_NAME, "ufmt3_theo_beta_analysis_%s_%s_branches_overhead_%s.csv"%(args.model_name, args.n_branches, args.overhead))
	...
	threshold_list = [0.8, 0.82, 0.82, 0.86, 0.88,0.89]
