...
def extractGlobalTSTemperature(args, temp_data_path, threshold, n_branches_edge):
	...
	df_temp = pd.read_csv(temp_data_path)
	df_temp = df_temp[(df_temp.threshold==0.7) & (df_temp.n_branches == n_branches_edge)]
...
def main(args):
	...
	resultsPath = os.path.join(config.DIR_NAME, "theoretical_beta_analysis_%s_%s_branches_%s_with_overhead_with_nano_with_test_set_pos_5_review_%s_overhead_%s.csv"%(args.model_name, args.n_branches, args.model_id, mode, args.overhead))
