...
def runGlobalTemperatureScalingInference(args, df_inf_data, df_val_inf_data, df_inf_data_device, threshold, n_branches_edge, savePath, temperature_overall, overhead, calib_mode):
...
def main(args):
	...
	resultsPath = os.path.join(config.DIR_NAME, "theoretical_beta_analysis_%s_%s_branches_%s_with_overhead_with_nano_with_test_set_FINAL_FINAL.csv"%(args.model_name, args.n_branches, args.model_id))
	...
	beta_list = np.round(np.arange(0.48, -0.01, -0.01), 2)
	#beta_list = [0.48]
