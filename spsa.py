...
class SPSA (object):
	...
	def min(self, report_interval=100):
		...
		while ((n_iter < self.max_iter) and (np.linalg.norm(theta_saved-theta)/np.linalg.norm(theta_saved)))
			...
			print("Iter: %s, Best Loss: %s, Best Theta: %s."%(n_iter, loss, best_loss, best_theta))
